...
function printTable(v, maxDepth = 4) {
    __maxDepth = maxDepth;
...
function _printTable(v, depth = 0) {
    ...
    const content = size === 0
        ? `<empty>`
        : depth > __maxDepth
            ? `<${size !== null && size !== void 0 ? size : 'unknown amount of'} item${size === 1 ? '' : 's'}${isArray && v.length !== size ? `; ${v.length} length` : ''}>`
            : '\n' + Array.from(iter, ([k, v]) => {
...
//# sourceMappingURL=print-maps.js.map